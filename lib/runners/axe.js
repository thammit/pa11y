...
runner.run = async options => {
	...
	async function runAxeCore() {
		...
		window.axe.configure(getAxeConfiguration());
	...
	function getAxeConfiguration() {
		...
		if (options.translations && options.translations.axe) {
			const translations = options.translations.axe;

			if (Object.prototype.hasOwnProperty.call(translations, options.language)) {
				axeConfiguration.locale = translations[options.language];
			}
		}
