...
runner.run = async options => {
	...
	function runHtmlCodeSniffer() {
		return new Promise((resolve, reject) => {
			...
			if (options.translations && options.translations.htmlcs) {
				const translations = options.translations.htmlcs;

				if (Object.prototype.hasOwnProperty.call(translations, options.language)) {
					window.translation[options.language] = translations[options.language];
				}
			}
