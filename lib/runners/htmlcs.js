...
runner.run = async options => {
	...
	function runHtmlCodeSniffer() {
		return new Promise((resolve, reject) => {
			...
			if (options.language) {
				const file = `../../translations/htmlcs/${options.language}.js`;

				options.log.debug(`Language option found: ${options.language}`);

				if (fs.existsSync(path.join(__dirname, file))) {
					options.log.debug(`Loading language file (${options.language}) for runner: ${runner}`);
					require(file);
				} else {
					options.log.debug(`Language file for ${options.language} not found.`);
				}
			}
