...
async function setPage(url, options, state) {
	...
	options.log.debug('Checking for headers...');
	if (options.headers && Object.keys(options.headers).length) {
		options.log.debug('Adding headers to puppeteer');
		await state.page.setExtraHTTPHeaders(options.headers);
	}
...
async function injectRunners(options, state) {
	...
	if (options.language) {
		options.log.debug(`Using locale ${options.language} for output`);
	} else {
		options.log.debug(`No locale set, using defaults`);
	}
	...
	for (const runner of options.runners) {
		if (!runnersJavascript[runner]) {
			...
			if (options.translations && options.translations[runner]) {
				options.log.debug(`Checking translation: Locale data for ${runner} exists`);
			} else {
				options.log.debug(`Checking translation: Locale data for ${runner} not found`);
			}
